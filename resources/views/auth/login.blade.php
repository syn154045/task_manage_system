<x-layouts.global title="SignIn">
    ...
    <div class="w-11/12 mx-auto bg-gray-100 flex items-center justify-center rounded-lg">
        <div class="w-4/5 max-w-screen-sm p-6">
            ...
            <h1 class="text-sm font-semibold mb-6 text-gray-500 text-center">Let's go outside and watch wild birds now!</h1>
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-4">
                ...
                <div class="relative">
                    ...
                    <div class="text-sm text-gray-600 text-center absolute top-0 right-0">
                        <p>
                            <a href="{{ route('password.request') }}" class="text-green-600 rounded-md hover:underline focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-700">
                                Forgot Password?
                            </a>
                        </p>
                    </div>
                    @error('password')
                        <div class="text-xs relative block w-fulzl rounded-lg border-red-500 text-red-700 opacity-90">
                            <div class="mr-12 tex-red-700">{{ $message }}</div>
                        </div>
                    @enderror
                </div> 
                ...
                <div>
                    <div class="form-check">
                        <label for="remember" class="form-check-label">
                            <input type="checkbox" name="remember" id="remember" {{ old('remember') ? 'checked' : '' }} class="form-check-input text-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300">
                            Remember Me
                        </label>
                    </div>
                </div>
            ...
            <div class="mt-4 text-sm text-gray-600 text-center">
                <p>
                    ...
                    <a href="{{ route('register') }}" class="text-green-600 rounded-md hover:underline focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-700">
                        Create an account
                    </a>
                </p>
            </div>
